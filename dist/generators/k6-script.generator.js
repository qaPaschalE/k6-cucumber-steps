...
exports.K6ScriptGenerator = void 0;
class K6ScriptGenerator {
    generateK6File(scenarios, metadata, config) {
        const header = `
/**
  ...
  return reports;
    ...
    generateOptions(metadata) {
        ...
        for (const meta of metadata) {
            if (meta.vus)
                vus = Math.max(vus, meta.vus);
            if (meta.duration)
                duration = meta.duration;
            if (meta.thresholds)
                Object.assign(thresholds, meta.thresholds);
            if (meta.stages) {
    ...
    generateTestFunction(scenarios, metadata) {
        const allStepCalls = scenarios
            ...
            if (isBrowser) {
                ...
                stepLines.push(`      if (!context) throw new Error("Browser Context could not be created.");`);
            ...
            scenario.steps.forEach((step) => {
                ...
                if (args && args.trim().length > 0)
                    callParams.push(args);
                if (step.argument) {
                    callParams.push(JSON.stringify(step.argument));
                }
                const awaitPrefix = isBrowser ? "await " : "";
                const indent = isBrowser ? "          " : "    ";
                stepLines.push(`${indent}${awaitPrefix}steps.${name}(${callParams.join(", ")});`);
            });
            if (isBrowser) {
                stepLines.push(`        } catch (err) {`);
                stepLines.push(`          console.error('Step Execution Error: ' + err.message);`);
                ...
                stepLines.push(`      console.error('Browser Initialization Failed: ' + e.message);`);
                stepLines.push(`    }`);
            }
...
exports.K6ScriptGenerator = K6ScriptGenerator;
//# sourceMappingURL=k6-script.generator.js.map